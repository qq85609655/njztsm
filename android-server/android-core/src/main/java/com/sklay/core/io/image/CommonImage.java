...
public class CommonImage implements FileOperation {
	...
	private ImageOperation imageOperation = ImageOperationFactory
			.getImageOperation();
	...
	private void convertToBufferedImage(InputStream inputStream)
			throws IOException {
		if (inputStream != null) {
	...
	public CommonImage resize(double ratio) {
		return new CommonImage(imageOperation.resize(bufferedImage, ratio),
				this.formatName);
	}

	public CommonImage crop(int left, int top, int width, int height) {
		return new CommonImage(imageOperation.crop(bufferedImage, left, top,
				width, height), this.formatName);
	}

	public CommonImage resizeWithContainer(int containerWidth,
			int containerHeight) {
		return new CommonImage(imageOperation.resizeWithContainer(
				bufferedImage, containerWidth, containerHeight),
				this.formatName);
	}

	public CommonImage cropWithContainer(int containerWidth, int containerHeight) {
		return new CommonImage(imageOperation.cropWithContainer(bufferedImage,
				containerWidth, containerHeight), this.formatName);
	}

	public CommonImage resizeWithMaxWidth(int maxWidth) {
		return new CommonImage(imageOperation.resizeWithMaxWidth(bufferedImage,
				maxWidth), this.formatName);
	}

	public CommonImage resizeWithMaxHeight(int maxHeight) {
		return new CommonImage(imageOperation.resizeWithMaxHeight(
				bufferedImage, maxHeight), this.formatName);
	}
	...
	public static void main(String[] args) {
		try {
			new CommonImage(new URI(
					"http://www.baidu.com/img/baidu_jgylogo3.gif"))
					.save("E:\\baidu.jpg");

			// CommonImage commonImage = new CommonImage("E:\\aaa.jpg");
			// commonImage.cropWithContainer(120,
			// 40).save("E:\\headtest-middle-cut.jpg");
			// commonImage.cropWithContainer(30,
			// 40).save("E:\\headtest-middle-cut2.jpg");
			// commonImage.cropWithContainer(220,
			// 220).save("E:\\headtest-middle-cut3.jpg");
			// commonImage.cropWithContainer(200,
			// 80).save("E:\\aaa-middle-cut1.jpg");
			// commonImage.cropWithContainer(110,
			// 220).save("E:\\aaa-middle-cut4.jpg");
		} catch (IOException e) {
			e.printStackTrace();
