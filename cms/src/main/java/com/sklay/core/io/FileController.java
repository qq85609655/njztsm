...
public class FileController implements ServletContextAware,
	...
	public String upload(MultipartFile file, String owner, String dir)
			throws IOException {
	...
	private String uploadAndGetUrl(MultipartFile file, String owner)
			throws IOException {
	...
	private String generatePathByOwner(MultipartFile file, String owner) {
		Map<String, FilePathGenerator> filePathGenerators = applicationContext
				.getBeansOfType(FilePathGenerator.class);
		if (!CollectionUtils.isEmpty(filePathGenerators)) {
			Iterator<FilePathGenerator> pathIterator = filePathGenerators
					.values().iterator();
		...
		return File.separator + owner + generateDataFolder() + File.separator
				+ file.getOriginalFilename();
	}
