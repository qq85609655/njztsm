...
public class AwtImageOperation implements ImageOperation {
	...
	public BufferedImage crop(BufferedImage src, int left, int top, int width,
			int height) {
		int iw = src.getWidth();
		int ih = src.getHeight();
		if (left + width > iw) {
		...
		g.drawRenderedImage(src,
				AffineTransform.getTranslateInstance(-left, -top));
		g.dispose();
		return dst;
	}

	public BufferedImage resizeWithContainer(BufferedImage src,
	...
	public BufferedImage cropWithContainer(BufferedImage src,
			int containerWidth, int containerHeight) {
		double iw = src.getWidth();
		double ih = src.getHeight();
		double t1 = iw * containerHeight;
