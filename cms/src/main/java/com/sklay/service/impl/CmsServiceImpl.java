...
import com.sklay.dao.GlobalDao;
import com.sklay.dao.PageDao;
import com.sklay.dao.WidgetDao;
...
public class CmsServiceImpl implements CmsService, InitializingBean,
	...
	public void changePageLayout(Page page, String newLayout) {
		if (!page.getLayout().equals(newLayout)) {
			...
			if (newRows.length < oldRows.length) {
				...
				List<Widget> widgets = /* page.getWidgets() */findWidgetsByPageId(page
						.getId());
				for (Widget widget : widgets) {
					String container = widget.getContainer();
					// skip 'row'
					int containerIndex = NumberUtils.parseNumber(
							container.substring(3), Integer.class);
	...
	public synchronized void saveGlobal(Global golbel) throws IOException {
		...
		FileUtils.writeStringToFile(globalToSave.getCss(), GLOBAL_CSS_LOCATION,
				servletContext);
		FileUtils.writeStringToFile(globalToSave.getJsp(), GLOBAL_JSP_LOCATION,
				servletContext);
	}
	...
	public void afterPropertiesSet() throws Exception {
		...
		if (global == null) {
			global = new Global();
			global.setCss(FileUtils.readFileToString(GLOBAL_CSS_LOCATION,
					servletContext));
			global.setJsp(FileUtils.readFileToString(GLOBAL_JSP_LOCATION,
					servletContext));
			saveGlobal(global);
		} else {
			FileUtils.writeStringToFile(global.getCss(), GLOBAL_CSS_LOCATION,
					servletContext);
			FileUtils.writeStringToFile(global.getJsp(), GLOBAL_JSP_LOCATION,
					servletContext);
		}
	...
	public List<Widget> findWidgetsByNameAndAttribute(String name,
		...
		return widgetDao.findByNameAndSettingsKeyAndSettingsValue(name,
				attrName, attrValue);
	}
}
