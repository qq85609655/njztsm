$(function() {
		$.extend({
			admin:function(args){
				...
				sklay.modal({id:"widgetEdit",header:"widget配置",
					onShown:function(){
						setTimeout(function(){
				...
				if(currentPageId){
					toolbar+= ("<li><a data-toggle='modal' href='"+ctx+"/page/edit?id="+currentPageId+"' data-target='#pageEdit'>编辑本页</a></li>"+
								...
								"<li><a data-toggle='modal' href='"+ctx+"/logout'>登出系统</a></li>";
				}
				toolbar+="</ul>";
				$(toolbar).appendTo("body").end().animate({"top":"30%"});
				sklay.modal({id:"pageCreate",header:"添加页面"});
				if(currentPageId){
					sklay.modal({id:"pageEdit",header:"编辑本页"});
					sklay.modal({id:"pageRemove",header:"删除本页"});
					sklay.modal({id:"widgetAdd",header:"添加模块",nofooter:true,onShown:function(){
					...
					sklay.modal({id:"layoutChange",header:"修改布局",nofooter:true,onShown:function(){
						setTimeout(function(){
							$(".layout-demo").hover(
								  function () {
				...
				sklay.modal({id:"globalEdit",header:"全局配置",onShown:function(){
