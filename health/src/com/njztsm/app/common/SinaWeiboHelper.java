...
public class SinaWeiboHelper {
	...
	private static final String CONSUMER_KEY = "1220571214";
	private static final String CONSUMER_SECRET = "91b2e17644e2ddbf135b30e1251a6cb2";
	private static final String REDIRECT_URL = "http://www.njztsm.net";
	...
	public static void setAccessToken(String accessKey, String accessSecret,
			long expiresIn) {
	...
	public static Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case OAUTH_RequestToken_ACCESS:
				Toast.makeText(context,
						context.getString(R.string.OAUTH_RequestToken_ACCESS),
						Toast.LENGTH_SHORT).show();
				break;
			case OAUTH_RequestToken_ERROR:
				Toast.makeText(context,
						context.getString(R.string.OAUTH_RequestToken_ERROR),
						Toast.LENGTH_SHORT).show();
				break;
			case OAUTH_AccessToken_ACCESS:
				Toast.makeText(context,
						context.getString(R.string.OAUTH_AccessToken_ACCESS),
						Toast.LENGTH_SHORT).show();
				break;
			case OAUTH_AccessToken_ERROR:
				Toast.makeText(context,
						context.getString(R.string.OAUTH_AccessToken_ERROR),
						Toast.LENGTH_SHORT).show();
				authorize(context, shareMessage, shareImage);// 跳转到授权页面
				break;
			case OAUTH_AccessToken_SXPIRED:
				Toast.makeText(context,
						context.getString(R.string.OAUTH_AccessToken_SXPIRED),
						Toast.LENGTH_SHORT).show();
				authorize(context, shareMessage, shareImage);// 跳转到授权页面
				break;
			case Weibo_Message_NULL:
				Toast.makeText(context,
						context.getString(R.string.Weibo_Message_NULL),
						Toast.LENGTH_SHORT).show();
				break;
			case Weibo_Message_LONG:
				Toast.makeText(context,
						context.getString(R.string.Weibo_Message_LONG),
						Toast.LENGTH_SHORT).show();
				break;
			case Weibo_Share_Success:
				Toast.makeText(context,
						context.getString(R.string.Weibo_Share_Success),
						Toast.LENGTH_SHORT).show();
				break;
			case Weibo_Share_Error:
				Toast.makeText(context,
						context.getString(R.string.Weibo_Share_Error),
						Toast.LENGTH_SHORT).show();
				break;
			case Weibo_Share_Repeat:
				Toast.makeText(context,
						context.getString(R.string.Weibo_Share_Repeat),
						Toast.LENGTH_SHORT).show();
				break;
			}
		};
		...
		weibo.authorize(cont, new WeiboDialogListener() {
			...
			public void onComplete(Bundle values) {
				try {
					...
					// 保存AccessToken
					AppConfig.getAppConfig(cont).setAccessInfo(
			...
			public void onError(DialogError e) {
				Toast.makeText(context, "授权失败 : " + e.getMessage(),
			...
			public void onWeiboException(WeiboException e) {
				Toast.makeText(context, "授权异常 : " + e.getMessage(),
						Toast.LENGTH_LONG).show();
			}
		});
	}
	...
	/**
	 * 微博分享 分享内容shareMessage & 分享图片shareImage & 当前Activity Toast会提示分享成功或失败
	 */
	public static void shareMessage(Activity cont, String shareMsg,
		...
		try {
			if (StringUtils.isEmpty(shareImage)) {
				weibo.shareToweibo(cont, accessToken.getToken(),
						accessToken.getSecret(), shareMessage);
			} else {
				// 分享带图片的微博
			}
	...
	/**
	 * 微博分享 分享内容shareMessage & 当前Activity Toast会提示分享成功或失败
	 */
	public static void shareMessage(Activity cont, String shareMsg) {
		shareMessage(cont, shareMsg, null);
	}
