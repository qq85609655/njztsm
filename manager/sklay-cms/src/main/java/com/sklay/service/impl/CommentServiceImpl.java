...
import com.sklay.core.web.OffsetLimitRequest;
...
public class CommentServiceImpl implements CommentService {
	...
	public void save(Comment comment, int fragmentSize, List<Long> tagIds) {
		...
		if (fragmentSize > 0) {
			commentToSave.setFragment(StringUtils.cut(
					commentToSave.getContent(), fragmentSize));
		}
	...
	public List<Comment> getLatest(List<String> owners, boolean onlyImg,
		...
		List<Comment> comments = cmsDao.getLatest(owners, onlyImg, start,
				end, offset, limit);
		resetComments(comments);
		return comments;
	}
	...
	public List<Comment> getMostCommented(List<String> owners, boolean onlyImg,
			Date start, Date end, int offset, int limit) {
		List<Comment> comments = cmsDao.getMostCommented(owners, onlyImg,
	...
	public List<Comment> getMostViewed(List<String> owners, boolean onlyImg,
			Date start, Date end, int offset, int limit) {
		List<Comment> comments = cmsDao.getMostViewed(owners, onlyImg,
				start, end, offset, limit);
		resetComments(comments);
		return comments;
	}
	...
	public List<Comment> getMostLiked(List<String> owners, boolean onlyImg,
			Date start, Date end, int offset, int limit) {
		List<Comment> comments = cmsDao.getMostLiked(owners, onlyImg,
				start, end, offset, limit);
		resetComments(comments);
		return comments;
	}
	...
	public void batchDeleteComments(List<Long> commentIds) {
		Iterable<Comment> list = commentDao.findAll(commentIds);
		if (null != list)
	...
	@Override
	public List<Comment> getCommentsByReferer(Long commentId, int offset,
			int limit) {
