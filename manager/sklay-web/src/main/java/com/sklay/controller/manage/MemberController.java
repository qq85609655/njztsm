...
public class MemberController
{
    ...
    public String list(Long groupId, String keyword, @PageableDefaults(value = 20)
    Pageable pageable, ModelMap modelMap)
    {
        ...
        Page<User> page = userAttrService.getUserPage(groupId, keyword, session, belong, pageable);
    ...
    @RequiresPermissions("member:create")
    @ResponseBody
    public DataView create(User user, Long groupId, String birth, ModelMap modelMap)
    {
        ...
        if (AuditStatus.PASS != targetGroup.getStatus())
            throw new SklayException(ErrorCode.AUTIT_ERROR, null, new Object[] {"会员分组", "添加会员"});
        ...
        if (userService.exist(user.getPhone()))
            throw new SklayException(ErrorCode.EXIST, null, "手机号码:" + user.getPhone(), "再次新增");
    ...
    public String initUpdate(@PathVariable
    Long userId, ModelMap modelMap)
    {
    ...
    @RequiresPermissions("member:update")
    @ResponseBody
    public DataView update(User user, String birth, Long groupId, ModelMap modelMap)
    {
        ...
        if (AuditStatus.WAIT == orginal.getStatus())
        {
            ...
            if (AuditStatus.PASS == targetGroup.getStatus())
                throw new SklayException(ErrorCode.AUTIT_ERROR, null, new Object[] {"会员分组", "修改会员分组"});
            ...
            if (!user.getPhone().equals(orginal.getPhone()) && userService.exist(user.getPhone()))
                throw new SklayException(ErrorCode.EXIST, null, "手机号码:" + user.getPhone(), "再次新增");
            
            BeanUtils.copyProperties(user, orginal, new String[] {"id", "status", "group"});
            
            if (MemberRole.AGENT == targetGroup.getRole() || MemberRole.ADMINSTROTAR == targetGroup.getRole())
                orginal.setPassword(PwdUtils.MD256Pws(orginal.getPhone().trim()));
            
        }
        else
        {
            BeanUtils.copyProperties(user, orginal, new String[] {"id", "status", "group", "phone"});
        }
    ...
    public String initBinding(@PathVariable
    Long userId, ModelMap modelMap)
    {
        ...
        if (null == userId)
            throw new SklayException(ErrorCode.FINF_NULL, null, new Object[] {"需绑定的会员"});
        ...
        if (AuditStatus.PASS != member.getStatus())
            throw new SklayException(ErrorCode.AUTIT_ERROR, null, new Object[] {member.getId(), "绑定设备"});
        ...
        if (MemberRole.ADMINSTROTAR == sesson.getGroup().getRole())
            ...
            bindings = bindingService.getUserBinding(owners, Level.FIRST, sesson);
    ...
    public DataView binding(Long userId, String[] devices)
    {
        ...
        User session = LoginUserHelper.getLoginUser();
        if (null == userId || null == devices || Constants.ZERO == devices.length)
            throw new SklayException(ErrorCode.FINF_NULL, null, new Object[] {"需绑定的会员或者设备号"});
        ...
        if (MemberRole.ADMINSTROTAR == session.getGroup().getRole())
            ...
            bindings = bindingService.getUserBinding(owners, Level.FIRST, session);
        ...
        List<DeviceBinding> deviceList = bindingService.getDefaultBindings(deviceSet, Level.FIRST);
            ...
            for (DeviceBinding device : deviceList)
            {
                ...
                if (null != device && device.getTargetUser().getId() != userId)
                    throw new SklayException(ErrorCode.EXIST, null, new Object[] {
                        "设备号:" + device.getSerialNumber() + "已存在" + Level.FIRST.getLable(),
                        "继续绑定" + Level.FIRST.getLable()});
                /** 当前设备与当前用户邦定信息已存在 */
                else if (null != device && device.getTargetUser().getId() == userId)
        ...
        for (String device : deviceSet)
        {
            if (!MobileUtil.isMobile(device))
                throw new SklayException(ErrorCode.ILLEGAL_PARAM, null, new Object[] {"设备号码格式不正确"});
            ...
            DeviceBinding binding =
                new DeviceBinding(device, member, setting.getDeviceBindingStatus(), Level.FIRST, operateTime,
    ...
    public String initMultipleBinding(Long[] userIds, ModelMap modelMap)
    {
        ...
        if (CollectionUtils.isNotEmpty(errorId))
            throw new SklayException(ErrorCode.AUTIT_ERROR, null, new Object[] {errorId, "绑定设备"});
    ...
    public DataView multipleBinding(Long[] userId, String sn, BindingMold mold)
    {
        ...
        if (null == mold)
            throw new SklayException(ErrorCode.MISS_PARAM, null, new Object[] {"设备类型"});
        
        if (null == userId || userId.length == 0)
            throw new SklayException(ErrorCode.MISS_PARAM, null, new Object[] {"需绑定的会员"});
        ...
        if (StringUtils.isBlank(sn))
            throw new SklayException(ErrorCode.MISS_PARAM, null, new Object[] {"需绑定的设备号"});
        ...
        if (targetUser.size() != members.size())
        {
            ...
            throw new SklayException(ErrorCode.FINF_NULL, null, new Object[] {"会员编号：" + targetUser + "的会员信息"});
        ...
        {
            long count = bindingService.getBindingUserCount(sn, null, mold, null);
        ...
        {
            DeviceBinding binding =
                new DeviceBinding(sn.trim(), member, status, Level.SECOND, operateTime, operateTime, session, operator,
                    mold);
            if (LoginUserHelper.isAdmin())
                binding.setBelong(session.getId());
            else
    ...
    Long userId, ModelMap modelMap)
    {
        ...
        List<DeviceBinding> deviceList = bindingService.getUserBinding(targetUser, null, null);
    ...
    @ResponseBody
    public DataView delete(@PathVariable
    Long userId)
    {
        
        if (null == userId)
            throw new SklayException(ErrorCode.FINF_NULL, null, new Object[] {"需删除的会员"});
        User member = userService.getUser(userId);
        ...
        if (AuditStatus.WAIT != member.getStatus())
            throw new SklayException(ErrorCode.EXIST, null, new Object[] {"会员的审核信息", "删除"});
    ...
    @RequiresUser
    public String initLineChart(@PathVariable
    Long userId, ModelMap modelMap)
    {
        if (null == userId)
            throw new SklayException(ErrorCode.FINF_NULL, null, new Object[] {"需绑定的会员"});
        ...
        List<MedicalReport> lastReports = reportService.getLastReports(userId, -7);
        ...
        for (MedicalReport mReoprt : lastReports)
        {
            ...
            Integer systolic_val = Integer.valueOf(gatherData.getHighPressure());
            Integer diastolic_val = Integer.valueOf(gatherData.getLowPressure());
            Integer health_val = gatherData.getIndexHealth();
            String reports_val = mReoprt.getResult();
            
            HealthReport healthReport = new HealthReport(systolic_val, diastolic_val, health_val, reports_val);
        ...
        modelMap.addAttribute("first_label", labels.get(0));
        modelMap.addAttribute("last_label", labels.get(labels.size() - 1));
        modelMap.addAttribute("userId", userId);
        
        return "modal:member.lineChart";
    }
    
    @RequestMapping("/lineChart/{userId}/{type}/{time}")
    @RequiresUser
    @ResponseBody
    public DataView lineChart(@PathVariable
    Long userId, @PathVariable
    String type, @PathVariable
    String time)
    {
        if (StringUtils.isBlank(time))
            throw new SklayException(ErrorCode.FINF_NULL, null, new Object[] {"会员体检时间"});
        
        Date dateTime = DateTimeUtil.getDate(time);
        
        if (StringUtils.isBlank(type) || (!"-".equals(type) && !"+".equals(type)))
            throw new SklayException(ErrorCode.ILLEGAL_PARAM, null, new Object[] {"类型参数"});
        ...
        json.put("labels", JSON.toJSONString(labels));
        json.put("systolic", JSON.toJSONString(systolic));
        json.put("diastolic", JSON.toJSONString(diastolic));
        json.put("health", JSON.toJSONString(health));
        json.put("reports", JSON.toJSONString(reports));
        json.put("vertical", 6);
        json.put("first_label", labels.get(0));
        json.put("last_label", labels.get(labels.size() - 1));
        json.put("userId", userId);
        
        DataView dataView = new DataView();
        dataView.setCode(1);
        dataView.setMsg("获取数据成功");
        dataView.setData(json.toJSONString());
    ...
    @ResponseBody
    public DataView remove(@PathVariable
    Long userId)
    {
        ...
        if (CollectionUtils.isNotEmpty(gList))
        {
            for (Group g : gList)
            {
                ...
                g.setDescription("来至于用户" + member.getName() + " Id为" + userId + "的分组");
                groups.add(g);
            }
        }
        groupService.update(groups);
        ...
        DeviceBinding mainBinding = bindingService.findTargetBinding(userId, Level.FIRST);
        if (null == mainBinding)
            bindingService.deleteTargetBinding(userId);
        else
            bindingService.deleteBinding(mainBinding.getSerialNumber());
        
        bindingService.updateBindingCreator(superUser, superUser.getId(), member);
    ...
    public String initAudit(@PathVariable
    Long userId, ModelMap modelMap)
    {
        modelMap.addAttribute("readonly", "readonly='readonly'");
        User member = userService.getUser(userId);
    ...
    @ResponseBody
    public DataView resetPwd(@PathVariable
    Long userId)
    {
        if (null == userId)
            throw new SklayException(ErrorCode.ILLEGAL_PARAM);
    ...
    private static User checkUser(User user, boolean checkGroup)
    {
        ...
        if (StringUtils.isNotBlank(user.getPassword()))
        {
            ...
            && (MemberRole.AGENT == user.getGroup().getRole() || MemberRole.ADMINSTROTAR == user.getGroup().getRole()))
            user.setPassword(PwdUtils.MD256Pws(user.getPhone().trim()));
        return user;
    }
    
    private User createMemberGroup(User member, GlobalSetting setting, User session)
    {
        
        if (SwitchStatus.OPEN != setting.getAutoCreateGroup())
            return member;
        
        if (null == session || null == session.getGroup() || null == session.getGroup().getRole())
            return member;
        
        MemberRole groupRole = session.getGroup().getRole();
        
        /** 超级管理员 */
        if (MemberRole.ADMINSTROTAR == groupRole || MemberRole.AGENT == groupRole)
        {
            
            if (MemberRole.AGENT == member.getGroup().getRole())
                groupService.create(autoInitGroup(member, session, setting.getUserAudite()));
        }
    ...
    private User createApplication(User member, User session)
    {
        ...
        if (null == member || null == member.getGroup() || null == member.getGroup().getRole())
            return member;
        
        MemberRole groupRole = member.getGroup().getRole();
        
        /** 超级管理员 */
        if (MemberRole.ADMINSTROTAR == groupRole && null != member.getGroup().getParentGroupId())
        {
    ...
    private static Set<Group> autoInitGroup(User member, User session, AuditStatus groupStatus)
    {
        
        Set<Group> groups = Sets.newHashSet();
        Long parentGroup = session.getGroup().getId();
        Group agentGroup =
            new Group(member.getId() + "下级代理商", MemberRole.AGENT, "下级代理商", member, groupStatus, parentGroup);
        agentGroup.setMemberCount(0L);
        Group memberGroup =
            new Group(member.getId() + "普通会员", MemberRole.MENBER, "普通会员", member, groupStatus, parentGroup);
    ...
    private static List<Application> autoInitApplication(User member, User session)
    {
        Date date = new Date();
        List<Application> apps = Lists.newArrayList();
        Application app_physical =
            new Application(AppType.PHYSICAL, AuditStatus.WAIT, 0, "", member.getId(), date, session.getId(), date);
        Application app_sos =
            new Application(AppType.SOS, AuditStatus.WAIT, 0, "", member.getId(), date, session.getId(), date);
    ...
    private void checkDeviceBinding(String sn, Set<Long> targetUser, Level level)
    {
        ...
        if (Level.FIRST == level)
        {
            ...
            if (null != binding)
                throw new SklayException(ErrorCode.EXIST, null, new Object[] {"设备号:" + sn + "已存在" + level.getLable(),
                    "继续绑定" + level.getLable()});
        }
        List<DeviceBinding> list = bindingService.getUserBindingByLevel(targetUser, null, sn);
        ...
        if (CollectionUtils.isNotEmpty(list))
        {
            ...
            throw new SklayException(ErrorCode.EXIST, null, new Object[] {
                "会员:" + targetUser.toString() + "已是绑定为" + level.getLable(), "再次绑定为" + level.getLable()});
        }
    }
