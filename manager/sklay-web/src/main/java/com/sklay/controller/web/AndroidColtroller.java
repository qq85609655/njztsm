...
public class AndroidColtroller
{
    ...
    public void android(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        try
        {
            transferTo(file, start, size - start, Channels.newChannel(response.getOutputStream()));
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    public DataView lastVersion()
    {
        ...
        // 更新描述信息
        String updateLog =
            "版本信息：3G智能健康服务机 for Android v" + versionName
                + " 更新日志：1、修复点击评论内容中链接时程序崩溃问题。请点击确定在线升级，如果升级失败，请到http://www.njztsm.net下载最新版本安装包大小：2.0MB";
        // 版本号
        int versionCode = 18;
        
        Update data = sklayApi.getMobileAndroidVer();
        DataView dataView = new DataView(1, "success", JSONObject.toJSONString(data));
    ...
    String ver, HttpServletRequest request, HttpServletResponse response)
    {
        String strDirPath = request.getSession().getServletContext().getRealPath("/");
        String name = "njzt-1.7.6.3.apk";
        String path = prePath(strDirPath) + "WEB-INF/version/" + name;
    ...
    public String initLineChart(String phone, ModelMap modelMap)
    {
        ...
        /** 获取绑定主号 */
        List<DeviceBinding> list = bindingService.findTargetBinding(phone.trim(), Level.FIRST);
        ...
        if (null == deviceBinding.getTargetUser())
            throw new SklayException(ErrorCode.SMS_GATHER_NOBANDING, null, phone);
        if (AuditStatus.PASS != deviceBinding.getStatus())
            throw new SklayException(ErrorCode.AUTIT_ERROR, null, new Object[] {"设备绑定信息", "获取体检报告"});
        User member = deviceBinding.getTargetUser();
        
        if (AuditStatus.PASS != member.getStatus())
            throw new SklayException(ErrorCode.AUTIT_ERROR, null, new Object[] {"用户信息", "获取体检报告"});
        ...
        List<MedicalReport> lastReports = reportService.getMemberReports(lastReport.getReportTime(), userId, -7);
        ...
        for (MedicalReport mReoprt : lastReports)
        {
            ...
            GatherData gatherData = JSONObject.parseObject(text, GatherData.class);
    ...
    @ResponseBody
    public DataView lineChart(String phone, String type, String time)
    {
        
        if (StringUtils.isBlank(phone))
            throw new SklayException(ErrorCode.SMS_GATHER_SIMNO_EMPTY);
        if (StringUtils.isBlank(type))
            throw new SklayException(ErrorCode.MISS_PARAM, null, new Object[] {"类型"});
        /** 获取绑定主号 */
        List<DeviceBinding> list = bindingService.findTargetBinding(phone.trim(), Level.FIRST);
        ...
        if (StringUtils.isBlank(time))
        {
            ...
            reporTime = lastReport.getReportTime();
        }
        else
        {
        ...
        Line line_systolic = new Line("收缩压（高压）", systolic, getRandColorCode(), line_width);
        
        Line line_diastolic = new Line("舒张压（低压）", diastolic, getRandColorCode(), line_width);
    ...
    private void transferTo(File file, long position, long count, WritableByteChannel target)
        throws IOException
    {
