...
import com.sklay.core.io.FileUtils;
...
import com.sklay.model.Widget;
import com.sklay.service.CmsService;
...
public class CmsController implements ServletContextAware, InitializingBean {
	...
	@RequestMapping("/")
	public String welcome(final NativeWebRequest request, final Locale locale,
			ModelMap modelMap) throws InterruptedException, TemplateException,
			IOException {
	...
	public String page(@PathVariable String alias,
			final NativeWebRequest request,
			final ModelAndViewContainer container, final Locale locale,
	...
	// render page
	private String renderPage(Page page, final NativeWebRequest request,
			final Locale locale, final ModelMap sharedModel)
			throws InterruptedException, TemplateException, IOException {
		...
		List<Widget> widgets = /* page.getWidgets(); */cmsService
				.findWidgetsByPageId(page.getId());
		...
		for (int i = 0; i < fs; i++) {
			...
			executor.submit(new Callable<Widget>() {
				...
				public Widget call() throws Exception {
					...
					try {
						// render
						renderWidget(sharedModel, widget, request, locale,
								false, null);
						// put
		...
		String content = FreeMarkerTemplateUtils.processTemplateIntoString(
				layoutConfiguration.getTemplate(page.getLayout(), locale),
				lmodel);
	...
	public String pageCreate(Page page, boolean copy, boolean isChildren,
			@RequestParam(required = false, defaultValue = "0") Long pageId) {
		if (isChildren) {
	...
	private void loopSetPage(Page page, Page sourcePage) {
		...
		List<Widget> widgets = /* sourcePage.getWidgets() */cmsService
				.findWidgetsByPageId(sourcePage.getId());
		if (!CollectionUtils.isEmpty(widgets)) {
			...
			for (Widget widget : widgets) {
				...
				if (!CollectionUtils.isEmpty(widget.getSettings())) {
					newWidget.setSettings(new HashMap<String, String>(widget
							.getSettings()));
				}
	...
	public boolean widgetAdd(Long pageId, String widgetName,
			final NativeWebRequest request,
			final ModelAndViewContainer container) throws Exception {
		Page page = cmsService.findPageById(pageId);
		...
		if (onAdd != null) {
			handleWidgetEvent(widgetInfo.getHandler(), onAdd, widget, request,
					container);
		}
		return true;
	}
	...
	@ResponseBody
	public boolean widgetRemove(Long widgetId, final NativeWebRequest request,
			final ModelAndViewContainer container) throws Exception {
		...
		if (widgetInfo != null) {
			...
			if (onRemove != null) {
				handleWidgetEvent(widgetInfo.getHandler(), onRemove, widget,
						request, container);
			}
		}
	...
	public String widgetEdit(Long widgetId, ModelMap modelMap,
		...
		WidgetInfo widgetInfo = findWidgetInfoByWidgetName(widget.getName());
		Map<String, String> mergedSettings = mergeSettings(
				widgetInfo.getSettingsMap(), widget.getSettings());
		modelMap.put("mergedSettings", mergedSettings);
		modelMap.put(
				"settings",
				prepare(widget, widgetInfo.getHandler(),
						widgetInfo.getSettings(), request, container));
		return "alone:core.widgetEdit";
	}

	private List<Setting> prepare(Widget widget, Object handler,
			List<Setting> settings, final NativeWebRequest request,
			final ModelAndViewContainer container) throws Exception {
		if (CollectionUtils.isEmpty(settings)) {
			...
			for (Setting setting : settings) {
				...
				if (optionsLoader == null) {
					...
					BeanUtils.copyProperties(setting, s,
							new String[] { "options" });
					InvocableHandlerMethod handlerMethod = createInitBinderMethod(
							handler, optionsLoader);
					Object result = handlerMethod.invokeForRequest(request,
							container, widget);
					if (result != null) {
						Option[] options = (Option[]) ObjectUtils
	...
	@ResponseBody
	public boolean widgetEdit(@RequestBody Widget widget,
			final NativeWebRequest request,
			final ModelAndViewContainer container) throws Exception {
		...
		if (onEdit != null) {
			handleWidgetEvent(widgetInfo.getHandler(), onEdit, stored, request,
					container);
		}
		return true;
	}
	...
	private void handleWidgetEvent(Object handler, Method method,
			Widget widget, final NativeWebRequest request,
			final ModelAndViewContainer container) throws Exception {
		InvocableHandlerMethod handlerMethod = createInitBinderMethod(handler,
				method);
	...
	public String widget(@PathVariable Long widgetId, boolean noborder,
			String border, final NativeWebRequest request, final Locale locale,
	...
	private void renderWidget(ModelMap sharedModel, Widget widget,
			final NativeWebRequest request, final Locale locale,
			boolean noborder, String borderClass) {
		long start = System.currentTimeMillis();
		String[] regionAndName = StringUtils.split(widget.getName(), ".");
		WidgetInfo widgetInfo = widgetManager.getWidgetInfo(regionAndName[0],
				regionAndName[1]);
		String content = null;
		if (widgetInfo == null) {
			content = "The widget " + widget.getName()
					+ " is missing,add related jars or remove it?";
			...
			ServletInvocableHandlerMethod handlerMethod = createServletInvocableHandlerMethod(
					handler, widgetInfo.getMethod());

			ModelAndViewContainer container = new ModelAndViewContainer();
			try {
				widget.setSettings(mergedSettings);

				Object mtpl = handlerMethod.invokeForRequest(request,
						container, widget);

				ModelMap model = container.getModel();
				model.addAllAttributes(sharedModel);
				String ctx = urlPathHelper.getContextPath(request
						.getNativeRequest(HttpServletRequest.class));
				// common attrs
				model.addAttribute("ctx", ctx);
				model.addAttribute("view", ctx
						+ "/view/"
						+ cmsService.findPageById(widget.getPageId())
								.getAlias());
				model.addAttribute("action", ctx + "/action/" + widget.getId());
				model.addAttribute("resource", ctx + "/widgetResource/"
						+ regionAndName[0]);
				...
				if (mtpl.getClass().isAssignableFrom(Template.class)) {
					...
					// render content
					Configuration widgetConfiguration = widgetManager
							.getConfiguration(handler);
					fmTemplate = widgetConfiguration.getTemplate((String) mtpl,
							locale);
				}
		...
		if (!noborder) {
			...
			if (borderTpl != null) {
				...
				if (!StringUtils.hasText(title)) {
					widget.setTitle(messageSource.getMessage(widget.getName(),
							null, locale));
				}
				...
				try {
					content = FreeMarkerTemplateUtils
							.processTemplateIntoString(borderConfiguration
									.getTemplate(borderTpl, locale), cmodel);
		...
		LOGGER.info("Widget {} rendered ,it cost {} ms",
				new Object[] { widget.getId(), end - start });
	}

	private Map<String, String> mergeSettings(
			Map<String, String> defaultSettings, Map<String, String> settings) {
		if (CollectionUtils.isEmpty(settings)) {
	...
	@Transactional
	public String renderPageByWidget(@PathVariable String parentPageAlias,
			final NativeWebRequest request, final HttpServletResponse response,
	...
	@Transactional
	public ModelAndView actionByWidget(@PathVariable Long widgetId,
			@PathVariable String region, @PathVariable String methodName,
			final NativeWebRequest request,
			final ModelAndViewContainer container,
			final HttpServletResponse response, Locale locale) throws Exception {
		...
		WidgetInfo widgetInfo = widgetManager.getWidgetInfo(regionAndName[0],
				regionAndName[1]);
		Map<String, String> mergedSettings = mergeSettings(
				widgetInfo.getSettingsMap(), actionFrom.getSettings());
		...
		if (method != null) {
			...
			if (WebUtils.isAjaxRequest(request
				...
				if (result.getClass().isAssignableFrom(String.class)
					...
					Configuration widgetConfiguration = widgetManager
							.getConfiguration(handler);
					Template fmTemplate = widgetConfiguration.getTemplate(
							(String) result, locale);
					WebUtils.writeToResponse(FreeMarkerTemplateUtils
							.processTemplateIntoString(fmTemplate,
									container.getModel()), response);
				} else {
		...
		return new ModelAndView(new RedirectView("/" + page.getAlias()),
				container.getModel());
	}
	...
	private InvocableHandlerMethod createInitBinderMethod(Object bean,
		...
		InvocableHandlerMethod binderMethod = new InvocableHandlerMethod(bean,
				method);
		binderMethod
				.setHandlerMethodArgumentResolvers(requestMappingHandlerAdapter
	...
	private WebDataBinderFactory getWebDataBinderFactory(Object handler) {
		return new ServletRequestDataBinderFactory(null,
				requestMappingHandlerAdapter.getWebBindingInitializer());
	}

	private ServletInvocableHandlerMethod createServletInvocableHandlerMethod(
		...
		servletInvocableHandlerMethod
				.setDataBinderFactory(getWebDataBinderFactory(handler));
		servletInvocableHandlerMethod
				.setHandlerMethodArgumentResolvers(requestMappingHandlerAdapter
						.getArgumentResolvers());
		servletInvocableHandlerMethod
				.setHandlerMethodReturnValueHandlers(requestMappingHandlerAdapter
