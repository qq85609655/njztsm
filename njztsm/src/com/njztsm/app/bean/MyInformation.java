...
public class MyInformation extends Entity {
	...
	public static MyInformation parse(InputStream stream) throws IOException,
			AppException {
				...
				case XmlPullParser.START_TAG:
					...
					if (tag.equalsIgnoreCase("user")) {
						...
						if (tag.equalsIgnoreCase("name")) {
							...
							user.setGender(StringUtils.toInt(
							...
							user.setFanscount(StringUtils.toInt(
									xmlParser.nextText(), 0));
						} else if (tag.equalsIgnoreCase("followerscount")) {
							user.setFollowerscount(StringUtils.toInt(
									xmlParser.nextText(), 0));
						}
						// 通知信息
