		...
		try {
			...
			while (evtType != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.START_TAG:
					if (tag.equalsIgnoreCase(Constants.Post.NODE_START)) {
						post = new Post();
					} else if (post != null) {
						if (tag.equalsIgnoreCase(Constants.Post.NODE_ID)) {
							post.id = StringUtils
									.toInt(xmlParser.nextText(), 0);
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_TITLE)) {
							post.setTitle(xmlParser.nextText());
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_URL)) {
							post.setUrl(xmlParser.nextText());
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_FACE)) {
							post.setFace(xmlParser.nextText());
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_BODY)) {
							post.setBody(xmlParser.nextText());
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_AUTHOR)) {
							post.setAuthor(xmlParser.nextText());
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_AUTHORID)) {
							post.setAuthorId(StringUtils.toInt(
									xmlParser.nextText(), 0));
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_ANSWERCOUNT)) {
							post.setAnswerCount(StringUtils.toInt(
									xmlParser.nextText(), 0));
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_VIEWCOUNT)) {
							post.setViewCount(StringUtils.toInt(
									xmlParser.nextText(), 0));
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_PUBDATE)) {
							post.setPubDate(xmlParser.nextText());
						} else if (tag
								.equalsIgnoreCase(Constants.Post.NODE_FAVORITE)) {
