...
import com.njztsm.app.adapter.ListViewFavoriteAdapter;
import com.njztsm.app.bean.FavoriteList;
import com.njztsm.app.bean.Notice;
import com.njztsm.app.bean.Result;
import com.njztsm.app.bean.FavoriteList.Favorite;
...
public class UserFavorite extends BaseActivity {
	...
	private void initView() {
		...
		favorite_catalog_software.setOnClickListener(this.favoriteBtnClick(
				favorite_catalog_software, Constants.Favorite.TYPE_SOFTWARE));
		favorite_catalog_post.setOnClickListener(this.favoriteBtnClick(
				favorite_catalog_post, Constants.Favorite.TYPE_POST));
		favorite_catalog_code.setOnClickListener(this.favoriteBtnClick(
				favorite_catalog_code, Constants.Favorite.TYPE_CODE));
		favorite_catalog_blog.setOnClickListener(this.favoriteBtnClick(
				favorite_catalog_blog, Constants.Favorite.TYPE_BLOG));
		favorite_catalog_news.setOnClickListener(this.favoriteBtnClick(
				favorite_catalog_news, Constants.Favorite.TYPE_NEWS));

		favorite_catalog_software.setEnabled(false);
		curFavoriteCatalog = Constants.Favorite.TYPE_SOFTWARE;

		lvFavorite_footer = getLayoutInflater().inflate(
				R.layout.listview_footer, null);
		lvFavorite_foot_more = (TextView) lvFavorite_footer
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						if (view instanceof TextView) {
							...
							TextView title = (TextView) view
									.findViewById(R.id.favorite_listitem_title);
		...
		mlvFavorite.setOnScrollListener(new AbsListView.OnScrollListener() {
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				try {
					if (view.getPositionForView(lvFavorite_footer) == view
				...
				if (scrollEnd && curLvDataState == UIHelper.LISTVIEW_DATA_MORE) {
					...
					loadLvFavoriteData(curFavoriteCatalog, pageIndex,
							mFavoriteHandler, UIHelper.LISTVIEW_ACTION_SCROLL);
				}
			}

			public void onScroll(AbsListView view, int firstVisibleItem,
					int visibleItemCount, int totalItemCount) {
				mlvFavorite.onScroll(view, firstVisibleItem, visibleItemCount,
						totalItemCount);
			}
		});
		mlvFavorite
					...
					public boolean onItemLongClick(AdapterView<?> parent,
						...
						final Handler handler = new Handler() {
							public void handleMessage(Message msg) {
								if (msg.what == 1) {
									...
									UIHelper.ToastMessage(UserFavorite.this,
											res.getErrorMessage());
								} else {
									((AppException) msg.obj)
											.makeToast(UserFavorite.this);
								}
							}
						};
						final Thread thread = new Thread() {
							public void run() {
								...
								try {
									Result res = ac.delFavorite(uid, fav.objid,
											fav.type);
									msg.what = 1;
									msg.obj = res;
						...
						UIHelper.showFavoriteOptionDialog(UserFavorite.this,
								thread);
	 ...
	 *            动作标识
	 */
	private void loadLvFavoriteData(final int type, final int pageIndex,
		...
		new Thread() {
			public void run() {
				...
				if (action == UIHelper.LISTVIEW_ACTION_REFRESH
					...
					isRefresh = true;
				try {
					FavoriteList favoriteList = ((AppContext) getApplication())
							.getFavoriteList(type, pageIndex, isRefresh);
	...
	private View.OnClickListener favoriteBtnClick(final Button btn,
			final int catalog) {
